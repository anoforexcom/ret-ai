          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="bg-slate-50 p-6 md:w-5/12 border-r border-slate-100 flex flex-col">
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                <User className="h-4 w-4" /> {t('payment_modal.user_data')}
              </h4>
                ...
                {!currentCustomer ? (
                  <>
                    <div className="grid grid-cols-2 gap-2">
                      <input
                        type="text"
                        placeholder={t('payment_modal.form.name_placeholder')}
                        value={firstName}
                      ...
                      <input
                        type="text"
                        placeholder={t('payment_modal.form.surname_placeholder')}
                        value={lastName}
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="email"
                        placeholder={t('payment_modal.form.email_placeholder')}
                        value={email}
              ...
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                <Package className="h-4 w-4" /> {t('payment_modal.selection')}
              </h4>
              ...
              <div className="border-t border-slate-200 pt-4 mt-auto">
                <div className="flex justify-between items-baseline">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('payment_modal.total')}</span>
            ...
            <div className="p-6 md:w-7/12">
              <div className="mb-6">
                <h4 className="text-sm font-bold text-slate-900 mb-4">{t('payment_modal.payment_method')}</h4>
                    ...
                    >
                      ...
                      {method.type === 'balance' && currentCustomer && (
                        <span className={`text-[9px] font-bold mt-1 ${currentCustomer.balance >= selectedTier.price ? 'text-green-600' : 'text-red-400'}`}>
                          {t('payment_modal.balance_available', { amount: currentCustomer.balance.toFixed(2) })}
                        </span>
                      )}
                    </button>
                  ))}
                  ...
                  {activeMethod.type === 'balance' ? (
                    <div className="p-6 bg-indigo-50 rounded-2xl border border-indigo-100 text-center">
                      ...
                      {currentCustomer && currentCustomer.balance >= selectedTier.price ? (
                        ...
                        >
                          {isProcessing ? t('payment_modal.internal_balance.processing') : t('payment_modal.internal_balance.confirm')}
                        </button>
                      ) : (
                        <div className="p-4 bg-red-50 border border-red-100 rounded-xl text-red-600 text-xs font-bold flex items-center gap-2">
                          <AlertCircle className="h-4 w-4" /> {t('payment_modal.internal_balance.insufficient')}
                        </div>
                      )}
                    </div>
                      ...
                      {!mbWaySent ? (
                        <>
                          <input
                            type="tel"
                            required
                            placeholder={t('payment_modal.mbway.phone_placeholder')}
                            value={mbWayPhone}
                          ...
                          >
                            {isProcessing ? t('payment_modal.mbway.processing') : t('payment_modal.mbway.request_btn', { amount: selectedTier.price.toFixed(2) })}
                          </button>
                        </>
                      ) : (
                        <div className="text-center py-4 bg-green-50 rounded-2xl">
                          <p className="text-sm font-bold text-slate-900">{t('payment_modal.mbway.confirm_title')}</p>
                          <button type="submit" className="mt-4 w-full bg-slate-900 text-white py-3 rounded-xl font-bold">{t('payment_modal.mbway.confirmed_btn')}</button>
                        </div>
                      )}
                    </form>
                  ) : (
                    <form onSubmit={handleSubmit} className="space-y-4">
                      <input type="text" placeholder={t('payment_modal.card.number')} required className="block w-full border border-slate-200 rounded-xl py-3 px-4 text-sm font-bold" />
                      <div className="grid grid-cols-2 gap-4">
                        <input type="text" placeholder={t('payment_modal.card.date')} required className="block w-full border border-slate-200 rounded-xl py-3 px-4 text-sm font-bold" />
                        <input type="text" placeholder={t('payment_modal.card.cvc')} required className="block w-full border border-slate-200 rounded-xl py-3 px-4 text-sm font-bold" />
                      </div>
                      <button type="submit" disabled={isProcessing} className="w-full py-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-black">
                        {isProcessing ? t('payment_modal.card.processing') : t('payment_modal.card.pay_btn', { amount: selectedTier.price.toFixed(2) })}
                      </button>
                    </form>
                  )}
                </div>
