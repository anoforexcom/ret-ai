...
/**
 * Restaura e colore a imagem chamando a nossa API Route interna.
 * Isto resolve o erro de "API Key must be set" no browser.
 */
export const restoreImage = async (file: File | string): Promise<string> => {
  try {
    ...
    // Chamada à nossa API Route em vez de usar o SDK diretamente no browser
    ...
    if (!data.restoredImage) {
      throw new Error("O resultado da restauração está vazio.");
    }
