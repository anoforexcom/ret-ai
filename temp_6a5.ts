...
export default async function handler(req: any, res: any) {
  // Configura├º├úo de CORS se necess├írio (opcional para rotas internas na Vercel)
  ...
  try {
    ...
    if (!output) {
      return res.status(500).json({ error: 'O Replicate n├úo devolveu nenhum resultado.' });
    }

    // Retorna o URL diretamente para o frontend. 
    // Isso evita o download e reconvers├úo pesada no servidor (que causa timeout).
